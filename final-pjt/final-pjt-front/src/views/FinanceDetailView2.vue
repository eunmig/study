<template>
    ...
    <div>
      <table>
        ...
        <tr>
          ...
          <tr>
            <td colspan="4">
              <OptionList2 :options="itemOption" />
            </td>
          </tr>
        </tbody>
      </table>
  ...
  import OptionList2 from '../components/OptionList2.vue';
  ...
  const fetchData = async () => {
    try {
      const optionsResponse = await axios.get(`${financeStore.API_URL}/finance/saving-product-options/${route.params.id}/`, {
        headers: {
      ...
      const productResponse = await axios.get(`${financeStore.API_URL}/finance/saving-product/${route.params.id}/`, {
        headers: {
          Authorization: `Token ${token}`
        }
      })
  ...
  const toggleLike = async () => {
    try {
      const likeResponse = await axios.post(
        `${financeStore.API_URL}/finance/saving-like/${item.value.fin_prdt_cd}/`,
        null,
        {
          headers: {
            Authorization: `Token ${token}`
  ...
  onMounted(() => {
    fetchData();
    console.log('onMount: FinanceDetailView2');
  });
  </script>
